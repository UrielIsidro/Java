...
import com.itbulls.learnit.isidro.oop.exam.onlineshop.services.ProductManagementService;
import com.itbulls.learnit.isidro.oop.exam.onlineshop.services.impl.DefaultProductManagementService;
...
public class ProductCatalogMenu implements Menu {
	...
	public void start() {
		...
		while (true){
			...
			if(context.getLoggedInUser() == null) {
				menuType = new MainMenu();
				System.out.println("You are not logged in. Please, sign in or create new account");
				break;
			}
			
			if(op.equals(MainMenu.MENU_COMMAND)) {
				menuType = new MainMenu();
				break;
			}
			
			if(op.equalsIgnoreCase(CHECKOUT_COMMAND)) {
				if(context.getSessionCart() == null || context.getSessionCart().isEmpty()) {
					System.out.println("Your cart is empty. Please, add product to cart first " +
							"and then proceed with checkout");
				}
				else {
					menuType = new CheckoutMenu();
					break;
				}
				
			} else {
				Product productToAddToCart = fetchProduct(op);
	...
	public void printMenuHeader() {
		...
		System.out.println("Enter product id to add it to the cart or ‘menu’ if you want to " +
